...
export function ProductList({amt, layout, page = ""}){
    ...
    const itemsToRender = sortedProducts.slice(amt, -1);
    ...
    return(
        ...
        {layout === "flex" ? (
            ...
            {itemsToRender.map((product) => (
                <div key={product?.product_id}  className="min-w-[50%] md:min-w-[25%] lg:min-w-[20%] w-full border mr-2.5 ">
                    <Link to={`/products/${product?.slug}`}>
                        <div className="relative">
                            <img src={product?.images?.[0].replace("/upload/","/upload/w_300,h_300,c_fill/") || logo} width={300} height={300}></img>
                            {product.sale && (
        ...
        <div id="Product-List" className="grid gap-4 grid-cols-[repeat(auto-fit,minmax(200px,1fr))]">
            {itemsToRender.map((product) => {
                ...
                return(
                <div key={product?.product_id}  className="max-w-64 w-full h-fit mx-auto bg-pink-50 rounded-xl">
                    <Link to={`/products/${product?.slug}`}>
                    ...
                    <div className="">
                        <button onClick={() => { addToCart(product?.product_id) }} className="w-full flex justify-center items-center rounded-lg py-1.5 text-[12px] uppercase tracking-wide bg-pink-100"><FaCartShopping className="mr-3"/>
                            { qty === 0 ? (<div>ADD to Cart</div>) : (<div>{cart[product?.product_id]}</div>)}
                        </button>
                    </div>
                </div>
                )
